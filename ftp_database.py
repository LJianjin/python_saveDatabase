...
# ISFIRST = 1
Local_dir = '/root/testdata/2020/data'
ftp = FTP()
AllFile = []
def ftpInit():
    ...
    #关闭FTP客户端连接
    ftp.close()
    # logging.info('file: {}'.format(AllFile))
    # return True
    for x in AllFile:
...
def ftpDown(file, flag):
    ...
    try:
        ...
        # print ' ' 
        logging.info('error down {}'.format(e))
        return False
    finally:
        ftpDo.close()
...
def run(file_year, month_list):
    ...
    pattern = r'DAT_ASCII_[A-Z]+_T_(%s){1}.zip' % month_str
    # csv_file = 'DAT_ASCII_[A-Z]+_T_[0-9]+.zip'
    # logging.info('in ftp function')

    engine = create_engine('postgresql://postgres:Supwin999@#$@localhost:5432/test_forex')
    ...
    for subdir, dirs, files in os.walk(folder):
        ...
        for file in files:
            try:
                ...
                # print(' ')
                logging.info('error ZipFile {}'.format(e.args))
                continue
    logging.info('All File is save to database')
    cursor.close()
    connection.close()
...
if __name__ == "__main__":
    ...
    ISFIRST = 0
    # logging.info('test: {}'.format(ISFIRST))
    # schedule.every(10).seconds.do(ftpInit)
    schedule.every().days.at("05:30").do(ftpInit)
